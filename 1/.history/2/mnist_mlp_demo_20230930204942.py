...
class MNIST_MLP(object):
    ...
    def load_data(self, data_path, label_path):
        ...
        test_images = self.load_mnist(os.path.join("../mnist_data", "t10k-images-idx3-ubyte"), True)
        test_labels = self.load_mnist(os.path.join("../mnist_data", "t10k-labels-idx1-ubyte"), False)
    ...
    def load_model(self, param_dir):  # 加载参数
        ...
        self.net.loadParams(1, weigh2, bias2, self.filter_quant_params[1])

        weigh3 = np.transpose(params['w3'], [1, 0]).flatten().astype(np.float64)
        bias3 = params['b3'].flatten().astype(np.float64)
        self.net.loadParams(2, weigh3, bias3, self.filter_quant_params[2])
